...
(function (b) {
    ...
    b.extend(b.jqx._jqxGrid.prototype, {
        defineInstance: function () {
                    ...
                    if (ad != null) {
                        ...
                        if (this.autorowheight && (this.autoheight || this.pageable)) {
                            ...
                            if (X && w) {
                                ...
                                // begin C2 modified
                                if (typeof this._rowdetailscache[ad.boundindex+'_'] != 'undefined') {
                                    new_height = this._rowdetailscache[ad.boundindex+'_'].customHeight;
                                } else {
                                    new_height = ad.rowdetailsheight;
                                }
                                O += new_height;
                                // end C2 modified
                            }
            ...
            e.css("width", "100%");
            //E.height(d.rowdetailsheight); // C2 modified
            e[0].className = q;
            var p = this.getboundindex(d);
            var j = p + "_";
            if (this._rowdetailscache[j]
                && this._rowdetailscache[j].windowWidth == $(window).width()
                && this._rowdetailscache[j].scrollHeight != this.vScrollInstance.value) {
                // if (this._rowdetailscache[j]) { // C2 modified to disable caching as part of dynamic row height fix
                ...
                if (this.initrowdetails) {
                    if (this._rowdetailscache[j].element) {
                        ...
                        var new_height = this._rowdetailscache[j].customHeight; // C2 modified
                        E.height(new_height); // C2 modified
                        b(f).height(new_height); // C2 modified
                        this._rowdetailscache[j].scrollHeight = this.vScrollInstance.value; //C2 modified
                    }
                } else {
                    e[0].innerHTML = C
                }
            ...
            if (this.initrowdetails) {
                ...
                // begin C2 modified added code - dynamic height for row details
                var new_height = $('#' + this.element.id + 'rowDetails' + p).parent().height() + 20;
                b(f).height(new_height);
                E.height(new_height);
                this._rowdetailscache[j].customHeight = new_height;
                this._rowdetailscache[j].windowWidth = $(window).width();
                this._rowdetailscache[j].scrollHeight = this.vScrollInstance.value;
                this.visiblerows[p - this._startboundindex].rowdetailsheight = new_height;
                // end C2 modified added code
