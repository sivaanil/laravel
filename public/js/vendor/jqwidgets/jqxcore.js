...
(function (a) {
    ...
    a.extend(a.jqx.mobile, {
        _touchListener: function (h, f) {
                ...
                if (navigator.platform.toLowerCase().indexOf("win") != -1) {
                    if (f.indexOf("Windows Phone") >= 0 || f.indexOf("WPDesktop") >= 0 || f.indexOf("IEMobile") >= 0 || f.indexOf("ZuneWP7") >= 0) {
                        ...
                        if (f.indexOf("Touch") >= 0) {
                            ...
                            // C2 Modified, partial fix for Windows tablet support
                            var b = ("MSPointerDown" in window) || ("pointerdown" in window) || (window.navigator.pointerEnabled);
                            if (b) {
                                this.touchDevice = true;
                                return true
                            }
            ...
            // C2 Modified
            if (this.isWindowsPhone() || window.navigator.pointerEnabled) {
                var c = navigator.userAgent.toLowerCase();
                // if (c.indexOf("windows phone 8.1") != -1) {
                // C2 Modified
                if (c.indexOf("windows phone 8.1") != -1 || window.navigator.pointerEnabled) {
            ...
            var x = function (J) {
                // C2 Modified - added fixes for touch scrolling interfering with in-grid actions
                if(J.target.className.indexOf('jqx-grid-group-expand') != -1
                    || J.target.className.indexOf('jqx-grid-group-collapse') != -1
                    || J.target.className.indexOf('menu-cell') != -1) {
                    return;
                }
                if(typeof J.originalEvent != 'undefined' && J.originalEvent.pointerType == 'mouse') {
                    return;
                }
                // end C2 Modified
                if (!B.enableScrolling[D]) {
