...
class AddTestDevice(c2_test_case.C2TestCase):
    ...
    def test_add_device(self):
        ...
        AddTestDevice.open_add_device_dialog(self, driver)
        AddTestDevice.populate_build_info(self, driver, device_type_array[0], device_name_array[0], ip_address_array[0],
        ...
        try:
            WebDriverWait(driver, self.config.long_timeout * 3).until(
                expected_conditions.visibility_of_element_located((By.XPATH, "//div[@id='scanProgressWindowContent']/div/form/div/div[2]/div[2]"))
            )
        except TimeoutException:
            self.fail("Scan dialog still displaying after " + str(self.config.long_timeout * 3) + " seconds.")

        driver.find_element_by_xpath("//div[@id='scanProgressWindowContent']/div/form/div/div[2]/div[2]").click()
        try:
            WebDriverWait(driver, self.config.short_timeout).until(
                expected_conditions.invisibility_of_element_located((By.ID, "scanProgressWindowContent"))
            )
        except TimeoutException:
            self.fail("could not close the scan dialog")
